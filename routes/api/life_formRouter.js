...
lifeRouter.get("/get_member_dtls", async (req, res) => {
  ...
  var select =
    ...
    where = `a.mem_type = c.memb_type
      AND date(c.effective_dt) = (SELECT max(date(d.effective_dt)) FROM md_member_fees d WHERE a.mem_type = d.memb_type)
...
lifeRouter.get("/get_dependent_dtls_life", async (req, res) => {
  ...
  // console.log(data, "ooo");
  var select =
      "member_id,mem_type,dependent_dt,dependent_name,gurdian_name gurd_name,relation, min_no spou_min,dob spou_dob,blood_grp spou_blood_grp,memb_address spou_memb_address,ps spou_ps,phone_no spou_phone,email_id spou_email,memb_pic spou_pic,city_town_dist",
