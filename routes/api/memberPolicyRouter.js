...
memberPolicyRouter.post("/member_policy_dtls", async (req, res) => {
  var data = req.body;
  var select =
      "a.form_no,a.member_id,a.association,a.memb_name,a.policy_holder_type policy_holder_type_id,a.memb_flag AS status,b.unit_name,c.policy_holder_type",
    ...
    //   : null,
    whr = `a.association = b.unit_id AND a.policy_holder_type = c.policy_holder_type_id`;
...
memberPolicyRouter.post("/member_policy_dtls_view", async (req, res) => {
  var data = req.body;
  var select =
      "a.form_no,a.form_dt,a.policy_holder_type,a.member_id,a.association,a.memb_type,a.memb_oprn,a.memb_name,a.gender,a.dob,a.mem_address,a.phone_no,a.min_no,a.personel_no,a.memb_flag,a.dependent_name,a.spou_min_no,a.spou_dob,a.spou_phone,a.spou_gender,a.spou_address,a.dependent_flag,a.premium_type",
...
memberPolicyRouter.post("/update_member_policy_dtls", async (req, res) => { 
  ...
  if(res_dt.suc > 0 && res_dt.msg.length > 0){
    ...
    fields = `policy_holder_type = '${data.policy_holder}',stp_memb_name = '${escapeSQL(data.member)}',stp_memb_phone = ${data.mobile ? `'${data.mobile}'` : 'NULL'},stp_user_status = '${data.memb_flag == 'Y' ? 'A' : 'I'}',modified_by = '${data.user}',modified_at = '${datetime}'`
     values = null,
    whr = `form_no = '${data.form_no}'`,
    flag = 1;
  var res_dt_user = await db_Insert(table_name, fields, values, whr, flag);
  }

  if (data.depenFields_2 && data.depenFields_2.length > 0) {
