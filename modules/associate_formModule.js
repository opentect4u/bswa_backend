var dateFormat = require("dateformat"),
  path = require("path"),
  fs = require("fs");
const { db_Select, db_Insert, generateDBValue } = require("./MasterModule");
  ...
  accept_dt_cash: (data) => {
    return new Promise(async (resolve, reject) => {
      ...
      var table_name = "td_transactions",
        fields = `(form_no,trn_dt,trn_id,sub_amt,onetime_amt,adm_fee,donation,premium_amt,tot_amt,pay_mode,created_by,created_at)`,
        values = `('${data.formNo}','${datetime}','${trn_id}','${data.subscriptionFee_associate}','${data.subscriptionFee_associate_one}','${data.admissionFee_associate}','${data.donationFee_associate}','0','${tot_amt}','${data.payment}','${data.user}','${datetime}')`,
        where = null,
        flag = 0;
  ...
  accept_dt_cheque: (data) => {
    return new Promise(async (resolve, reject) => {
      ...
      var table_name = "td_transactions",
        ...
        values = `('${data.formNo}','${datetime}','${trn_id}','${data.subscriptionFee_associate}','${data.subscriptionFee_associate_one}','${data.admissionFee_associate}','${data.donationFee_associate}','0','${tot_amt}','${data.payment}','${data.cheque_no}','${data.cheque_dt}','${data.bank_name}','${data.user}','${datetime}')`,
        where = null,
        flag = 0;
      var res_dt = await db_Insert(table_name, fields, values, where, flag);
